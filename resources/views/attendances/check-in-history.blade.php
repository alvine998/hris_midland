...
@section('content')
<div class="max-w-4xl mx-auto px-0 sm:px-2 space-y-4 sm:space-y-6">
    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-0">
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Check-In History</h2>
            <p class="mt-0.5 sm:mt-1 text-sm text-gray-500 dark:text-gray-400">Your selfie attendance records</p>
        </div>
        <a href="{{ route('attendances.check-in') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 sm:px-4 py-3 sm:py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 active:bg-indigo-800">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @if ($attendance->is_mock_location_in)
                                <span class="ml-1.5 inline-flex items-center rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300">Suspicious</span>
                            @endif
                        </td>
        ...
        <div class="sm:hidden divide-y divide-gray-100 dark:divide-gray-700">
            @foreach ($attendances as $attendance)
            <div class="p-4 space-y-2 active:bg-gray-50 dark:active:bg-gray-700/50 transition-colors">
                {{-- Date + Hours row --}}
                ...
                <div class="flex items-center gap-4 text-sm">
                    <div class="flex items-center gap-1.5">
                        <span class="text-xs text-gray-400 dark:text-gray-500">In:</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ $attendance->clock_in?->format('H:i') }}</span>
                        @if ($attendance->is_mock_location_in)
                            <span class="inline-flex items-center rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300">Suspicious</span>
                        @endif
                    </div>
                    <div class="flex items-center gap-1.5">
                        <span class="text-xs text-gray-400 dark:text-gray-500">Out:</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ $attendance->clock_out?->format('H:i') ?? '-' }}</span>
                        @if ($attendance->clock_out && $attendance->is_mock_location_out)
                            <span class="inline-flex items-center rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300">Suspicious</span>
                        @endif
                    </div>
                ...
                <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span>
                        @if ($attendance->selfie_in)
                        <a href="{{ Storage::url($attendance->selfie_in) }}" target="_blank" class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-400 font-medium">
                        ...
                        @else
                        <span class="text-gray-400">No selfie</span>
                        @endif
                    </span>
                </div>
            </div>
