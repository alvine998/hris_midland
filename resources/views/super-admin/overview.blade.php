...
<div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Super Admin Overview</h2>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            @if ($selectedDivision)
                Viewing <span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $selectedDivision->name }}</span>@if($selectedDivision->department) · {{ $selectedDivision->department->name }}@endif
            @else
                Company-wide overview across all divisions.
            @endif
        </p>
    </div>
    <form method="GET" action="{{ route('admin.overview') }}" class="flex items-center gap-2">
        <label for="division_id" class="text-sm font-medium text-gray-700 dark:text-gray-300 shrink-0">Division</label>
        <select id="division_id" name="division_id" onchange="this.form.submit()"
            class="rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 min-w-[220px]">
            <option value="">All Divisions</option>
            @foreach ($divisions as $division)
                <option value="{{ $division->id }}" {{ $selectedDivision?->id === $division->id ? 'selected' : '' }}>
                    {{ $division->name }}{{ $division->department ? ' — '.$division->department->name : '' }} ({{ number_format($division->active_employees_count) }})
                </option>
            @endforeach
        </select>
        <button type="submit" class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 shrink-0">Apply</button>
    </form>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($metrics['activeEmployees']) }}</p>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Active Employees</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($metrics['presentToday']) }}</p>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Present Today</p>
    </div>

    {{-- On Leave Today --}}
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        <div class="flex items-center justify-between mb-4">
            <div class="w-10 h-10 bg-cyan-100 dark:bg-cyan-900/50 rounded-xl flex items-center justify-center">
                <svg class="w-5 h-5 text-cyan-600 dark:text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
            </div>
        </div>
        <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($metrics['onLeaveToday']) }}</p>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">On Leave Today</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($metrics['pendingLeaves']) }}</p>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Pending Leaves</p>
    </div>
    ...
    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ $selectedDivision ? 'Employees by Work Location' : 'Employees by Division' }}
            </h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Active employees grouped by {{ $selectedDivision ? 'assigned location' : 'division' }}</p>
        </div>
        <div class="h-72">
            <canvas id="headcountChart" class="h-full w-full"></canvas>
        </div>
    </div>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm mb-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Division Breakdown</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Active employees per division</p>
        </div>
        <a href="{{ route('master-data.divisions') }}" class="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors shrink-0">Manage Divisions</a>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    <th class="text-left px-4 py-3 font-semibold text-gray-900 dark:text-white">Division</th>
                    <th class="text-left px-4 py-3 font-semibold text-gray-900 dark:text-white">Department</th>
                    <th class="text-right px-4 py-3 font-semibold text-gray-900 dark:text-white">Active Employees</th>
                    <th class="text-right px-4 py-3 font-semibold text-gray-900 dark:text-white">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($divisions as $division)
                <tr class="{{ $selectedDivision?->id === $division->id ? 'bg-indigo-50/60 dark:bg-indigo-900/20' : '' }}">
                    <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $division->name }}</td>
                    <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $division->department?->name ?? '-' }}</td>
                    <td class="px-4 py-3 text-right text-gray-900 dark:text-white">{{ number_format($division->active_employees_count) }}</td>
                    <td class="px-4 py-3 text-right">
                        <a href="{{ route('admin.overview', ['division_id' => $division->id]) }}" class="text-indigo-600 dark:text-indigo-400 font-medium hover:text-indigo-500 dark:hover:text-indigo-300">View</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No divisions found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $selectedDivision ? 'Latest additions in this division' : 'Latest additions across all divisions' }}</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 py-3 border-b border-gray-100 dark:border-gray-700 last:border-0">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-900 dark:text-white">
                    ...
                    in {{ $employee->division?->name ?? $employee->department?->name ?? 'Unassigned Division' }}
                </p>
...
<script>
    (() => {
        const overviewCharts = @json($charts);
        ...
        const renderCharts = () => {
            drawBarChart('headcountChart', overviewCharts.headcount);
            drawDonutChart('attendanceStatusChart', overviewCharts.attendanceStatus, { centerLabel: 'Records' });
            drawDonutChart('employeeStatusChart', overviewCharts.employeeStatus, { centerLabel: 'Employees' });
            drawLineChart('monthlyJoinsChart', overviewCharts.monthlyJoins);
        };
