...
@section('title', 'Job Positions - ' . config('app.name'))

@section('content')
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Job Positions</h2>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage job positions and titles.</p>
    </div>
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.away="open = false">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white" x-text="edit ? 'Edit Job Position' : 'Add Job Position'"></h3>
                ...
                <form method="POST" :action="edit ? `/master-data/job-positions/${item.id}` : '{{ route('master-data.job-positions.store') }}'">
                    @csrf
                    <template x-if="edit">
                        @method('PUT')
                    </template>
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" x-model="item.name" required placeholder="Job position name" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Level</label>
                            <select name="level_id" x-model="item.level_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
...
<x-list-search :action="route('master-data.job-positions')" placeholder="Search job position or level" />
...
<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th class="text-left px-6 py-4 font-semibold text-gray-900 dark:text-white">Level</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td class="px-6 py-4 text-gray-900 dark:text-white font-medium">{{ $jobPosition->name }}</td>
                    <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $jobPosition->level?->name }}</td>
                    <td class="px-6 py-4 text-right">
                        <div x-data="{ open: false, edit: false, item: {} }" class="inline-flex items-center gap-1">
                            <button @click="open = true; edit = true; item = { id: {{ $jobPosition->id }}, name: @json($jobPosition->name), level_id: {{ $jobPosition->level_id ?? 'null' }} }" class="p-1.5 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors">
                            ...
                            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.away="open = false">
                                ...
                                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6">
                                    <div class="flex items-center justify-between mb-4">
                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Job Position</h3>
                                        <button @click="open = false" class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                        </button>
                                    </div>
                                    <form method="POST" :action="`/master-data/job-positions/${item.id}`">
                                        @csrf
                                        @method('PUT')
                                        <div class="space-y-4">
                                            <div>
                            ...
                            <div x-data="{ show: false, id: null }">
                                <button @click="show = true; id = {{ $jobPosition->id }}" class="p-1.5 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                                ...
                                <div x-show="show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.away="show = false">
                                    ...
                                    <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6 text-center">
                                        ...
                                        <form :action="`/master-data/job-positions/${id}`" method="POST" class="flex justify-center gap-3">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">{{ $jobPositions->links() }}</div>
    @endif
</div>
@endsection
