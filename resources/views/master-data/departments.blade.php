...
<div x-data="{ createModal: false, editModal: false, editItem: {}, deleteModal: false, deleteId: null }" class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage organizational departments.</p>
        </div>
    ...
    <x-list-search :action="route('master-data.departments')" placeholder="Search department or company" />
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th class="text-left px-6 py-4 font-semibold text-gray-900 dark:text-white">Company</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $department->company?->name }}</td>
                        <td class="px-6 py-4 text-right">
                            <button @click="editModal = true; editItem = { id: {{ $department->id }}, name: @js($department->name), company_id: @js($department->company_id) }" class="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium text-sm mr-3">Edit</button>
                            <button @click="deleteModal = true; deleteId = {{ $department->id }}" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 font-medium text-sm">Delete</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No departments found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="createModal || editModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="createModal = false; editModal = false">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-lg p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4" x-text="editModal ? 'Edit Department' : 'Add New Department'"></h3>
            <form :action="editModal ? `/master-data/departments/${editItem.id}` : '/master-data/departments'" method="POST">
                @csrf
                <input type="hidden" name="_method" x-bind:value="editModal ? 'PUT' : 'POST'">
                <div class="space-y-4">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Name</label>
                        <input type="text" name="name" id="name" x-model="editItem.name" placeholder="Department name" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div>
                        <label for="company_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Company</label>
                        <select name="company_id" id="company_id" x-model="editItem.company_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <div x-show="deleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="deleteModal = false">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6 text-center">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">Are you sure you want to delete this department? This action cannot be undone.</p>
            <form :action="`/master-data/departments/${deleteId}`" method="POST" class="flex justify-center gap-3">
                @csrf
                @method('DELETE')
