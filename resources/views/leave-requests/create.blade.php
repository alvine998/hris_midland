...
@section('title', 'Create Leave Request - ' . config('app.name'))
...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mt-2">Create Leave Request</h2>
    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Submit a new leave request for an employee.</p>
</div>
    ...
    <form method="POST" action="{{ route('leave-requests.store') }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Leave request title" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('title') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="date" name="start_date" value="{{ old('start_date') }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('start_date') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="date" name="end_date" value="{{ old('end_date') }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('end_date') border-red-500 @enderror">
                @error('end_date')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Inclusive Days</label>
                <input type="number" name="inclusive_days" value="{{ old('inclusive_days', 1) }}" min="0" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('inclusive_days') border-red-500 @enderror" placeholder="Number of days">
            ...
            <div>
                ...
                <select name="status" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('status') border-red-500 @enderror">
                    <option value="on_progress" @selected(old('status', 'on_progress') === 'on_progress')>On Progress</option>
            ...
            <div>
                ...
                <input type="text" name="evidence" value="{{ old('evidence') }}" placeholder="Evidence file path" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('evidence') border-red-500 @enderror">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="reason" rows="3" placeholder="Reason for leave" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('reason') border-red-500 @enderror">{{ old('reason') }}</textarea>
                @error('reason')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Notes</label>
                <textarea name="notes" rows="3" placeholder="Additional notes" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
        ...
        <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" class="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700">Create Leave Request</button>
        </div>
    </form>
</div>
@endsection
